...
def sweep_wallet(wa, t, s, prk, a):
	...
	if wa[0] == 'm' or wa[0] == 'n':	
		...
		txout = TxOutput(to_satoshis(s), Script(['OP_DUP', 'OP_HASH160', addr.to_hash160(), 'OP_EQUALVERIFY', 'OP_CHECKSIG']) ) 
...
def main():
	...
	canvas1.create_text(225, entry_y-140, fill="black",
	font="Arial 10 bold",
	text="CREATE TIMELOCK")
		




	canvas1.create_text(225, entry_y-95, fill="black",
	font="Arial 10",
	text="Generate a P2SH address")
	...
	canvas1.create_text(225, entry_y-10, fill="black",
	font="Arial 10",
	text="Sender's Wallet")

	

	canvas1.create_text(40, entry_y+15, fill="black",
	font="Arial 10",
	text="Priv")

	

	canvas1.create_text(40, entry_y+45, fill="black",
	font="Arial 10",
	text="Addr")
	...
	canvas1.create_text(225,entry_y+75,fill="black",
	font="Arial 10",
	text="Enter date: MM-DD-YYYY")
	...
	canvas1.create_text(225,entry_y+160,fill="black",
	font="Arial 10",
	...
	canvas1.create_text(350,entry_y+220,fill="black",
	font="Arial 10",
	text="BTC")
	...
	canvas1.create_text(125,entry_y+220,fill="black",
	font="Arial 10",
	text="VOUT")
	...
	def complete():
		...
		if timestamp.get() != "" and txid_input.get() != "" and amount.get() != "" and p2sh.get() != "" and sk.get() != "" and sa.get() != "":
			...
			if validate(iso) == True:
				...
				sender_redeem_script = Script([seq.for_script(), 'OP_CHECKLOCKTIMEVERIFY', 'OP_DROP', 'OP_DUP', 'OP_HASH160', sender_p2pkh_addr.to_hash160(), 'OP_EQUALVERIFY', 'OP_CHECKSIG'])

				sender_addr = P2shAddress.from_script(sender_redeem_script)
				
				sender_to_addr = P2pkhAddress(sender_pubk)
				sender_txout = TxOutput(to_satoshis(satoshis), sender_to_addr.to_script_pub_key())

				sender_tx = Transaction([sender_txin], [sender_txout], sender_lock.for_transaction())

				sender_sig = sender_p2pkh_sk.sign_input(sender_tx, 0, sender_redeem_script)
				

				sender_txin.script_sig = Script([sender_sig, sender_p2pkh_pk, sender_redeem_script.to_hex()])
				...
				rec_redeem_script = Script([seq.for_script(), 'OP_CHECKLOCKTIMEVERIFY', 'OP_DROP', 'OP_DUP', 'OP_HASH160', rec_p2pkh_addr.to_hash160(), 'OP_EQUALVERIFY', 'OP_CHECKSIG'])

				rec_addr = P2shAddress.from_script(rec_redeem_script)
			
				rec_to_addr = P2pkhAddress(rec_pubk)
				rec_txout = TxOutput(to_satoshis(satoshis), rec_to_addr.to_script_pub_key())

			
				rec_tx = Transaction([rec_txin], [rec_txout], rec_lock.for_transaction())

				rec_sig = rec_p2pkh_sk.sign_input(rec_tx, 0, rec_redeem_script)
				

				rec_txin.script_sig = Script([rec_sig, rec_p2pkh_pk, rec_redeem_script.to_hex()])
...
to a flash drive. DO NOT INCLUDE SENDER WALLET. Directions to redeem are included in their wallet.
				...
				sender_wallet.write('TxId: ' + f'{sender_txid}' +
							'\n\nPrivate key: ' + f'{sender_privk}' +
								'\n\nPublic address: ' + f'{sender_pubk}' +
								'\n\nTimelock address: ' + f'{p2sh_addr}' +
							'\n\nRedeem date: ' + f'{iso}' + 
							
							'\n\nRedeem script: ' + f'{sender_signed_tx}' +
...
this means the transaction is working as expected. Please wait a few hours and try again.
				...
				rec_wallet.write('TxId: ' + f'{rec_txid}' +
							'\n\nPrivate key: ' + f'{rec_privk}'  + 
							'\n\nPublic address: ' + f'{rec_pubk}' +
							'\n\nTimelock address: ' + f'{p2sh_addr}' +
							'\n\nRedeem date: ' + f'{iso}' + 
							
							'\n\nRedeem script: ' + f'{rec_signed_tx}' +
							'\n\n' + f'{rec_wallet_text}'
							)
				...
				label1 = Label(root, bg='white', text= '                 Success!                   ')
				...
				canvas1.create_text(225,entry_y+135,fill="black",
				font="Arial 10",
			...
			canvas1.create_text(225,entry_y+270,fill="black",
			font="Arial 10",
	...
	canvas1.create_line(0,entry_y+350, entry_y+390, entry_y+350, fill='black', width=5)

	canvas1.create_text(225, entry_y+370, fill="black",
	font="Arial 10 bold",
	text="SWEEP WALLET")
	...
	canvas1.create_text(225, entry_y+395, fill="black",
	font="Arial 10",
	...
	canvas1.create_text(225,entry_y+425,fill="black",
	font="Arial 10",
	...
	canvas1.create_text(225,entry_y+475,fill="black",
	font="Arial 10",
	text="Enter BTC amoumt")
	...
	def redeem():
		...
		if sat_enter.get() != "" and addr_enter.get() != "":
			...
			if rec_exists == False and sender_exists == False:
				...
				label1 = Label(root, bg='white', text= 'Missing wallet. Move wallet to this \nfolder and restart BTCapsule')
